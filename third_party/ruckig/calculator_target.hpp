...
private:
    ...
    bool is_input_collinear(const InputParameter<DOFs, CustomVector>& inp, Profile::Direction limiting_direction, size_t limiting_dof) {
        ...
        nonstd::optional<size_t> scale_dof; // Need to find a scale DOF because limiting DOF might not be phase synchronized
        for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
    ...
    bool synchronize(nonstd::optional<double> t_min, double& t_sync, nonstd::optional<size_t>& limiting_dof, Vector<Profile>& profiles, bool discrete_duration, double delta_time) {
    ...
    Result calculate(const InputParameter<DOFs, CustomVector>& inp, Trajectory<DOFs, CustomVector>& traj, double delta_time, bool& was_interrupted) {
        ...
        for (size_t dof = 0; dof < degrees_of_freedom; ++dof) {
            ...
            if (!inp.enabled[dof]) {
                ...
                blocks[dof].a = nonstd::nullopt;
                blocks[dof].b = nonstd::nullopt;
                continue;
            }
        ...
        nonstd::optional<size_t> limiting_dof; // The DoF that doesn't need step 2
